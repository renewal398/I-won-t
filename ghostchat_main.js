...
(function (window) {
  ...
  var ghostChatState = {
    ...
    widgetOpen: false,
    licenseApiUrl:
      "https://license-validator-1.onrender.com/api/validate-license", // Configure this
    scrapingApiUrl: "https://license-validator-t4f9.onrender.com/api/scrape", // Configure this
  };
  ...
  function initGhostChat(options) {
    ...
    if (ghostChatState.config.licenseKey) {
      validateLicense(
        ghostChatState.config.licenseKey,
        function (isValid, tier) {
          if (isValid) {
            ...
            console.warn(
              "[GhostChat] Invalid license - falling back to FREE tier"
            );
          ...
          if (
            ghostChatState.config.contextMode &&
            ghostChatState.config.contextUrl
          ) {
            loadContext(
              ghostChatState.config.contextMode,
              ghostChatState.config.contextUrl
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      try {
        ...
        if (
          ...
          internalLinks.indexOf(linkUrl.href) === -1
        ) {
  ...
  function extractFAQ(html) {
    ...
    for (var i = 0; i < headings.length; i++) {
      ...
      if (
        nextElement &&
        (nextElement.tagName === "P" || nextElement.tagName === "DD")
      ) {
  ...
  function renderWidget() {
    ...
    container.className =
      "ghostchat-container ghostchat-" + theme + " ghostchat-" + position;
    ...
    container.innerHTML =
      ...
      "</svg>" +
      "<span>" +
      ghostChatState.config.buttonText +
      "</span>" +
      "</div>" +
      ...
      '<div class="ghostchat-message-content">' +
      ghostChatState.config.welcomeMessage +
      "</div>" +
      "</div>" +
      "</div>" +
      ...
      "Powered by GhostChat • " +
      ghostChatState.tier.toUpperCase() +
      " tier" +
      "</div>" +
      "</div>";
  ...
  function handleOfflineMode(message) {
    ...
    if (lowerMessage.includes("hello") || lowerMessage.includes("hi")) {
      response =
        "Hello! I'm currently in offline mode. Please provide an API key to enable AI responses.";
    } else if (lowerMessage.includes("help")) {
      response =
        "I'd love to help! However, I need an API key to provide intelligent responses. Please configure your OpenAI API key.";
    } else if (
      lowerMessage.includes("email") ||
      lowerMessage.includes("contact")
    ) {
      response =
        "Thank you for your interest! To get in touch with us, please email support@example.com or visit our contact page.";
    } else if (
      ...
      response =
        "Thank you for your message. I'm currently in offline mode and cannot provide AI-powered responses. Please configure an API key or leave your email and we'll get back to you.";
    }
  ...
  function addLoadingMessage() {
    ...
    messageDiv.className =
      "ghostchat-message ghostchat-message-bot ghostchat-message-loading";
  ...
  function callOpenAI(message, systemContext, apiKey) {
    var model =
      ghostChatState.config.provider.split(":")[1] || "deepseek/deepseek-chat";
    ...
    var messages = [
      ...
      .concat(ghostChatState.messages.slice(0, -1))
  ...
  function callAnthropic(message, systemContext, apiKey) {
    var model =
      ghostChatState.config.provider.split(":")[1] ||
      "claude-3-sonnet-20240229";

    var messages = ghostChatState.messages
      .slice(0, -1)
      .concat([{ role: "user", content: message }]);
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4) {
        ...
        if (xhr.status === 200) {
          ...
          addMessage(
            "Sorry, there seems to be an authentication issue. Please check your API key configuration.",
            "bot"
          );
